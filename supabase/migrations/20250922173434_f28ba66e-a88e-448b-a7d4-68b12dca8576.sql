...
CREATE INDEX IF NOT EXISTS idx_facts_geolocation_gist 
ON public.facts USING GIST (geolocation);

CREATE INDEX IF NOT EXISTS idx_facts_location_coords 
ON public.facts (latitude, longitude) 
WHERE latitude IS NOT NULL AND longitude IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_facts_status_location 
ON public.facts (status, latitude, longitude) 
WHERE status IN ('verified', 'pending');

-- Create optimized clustering function
